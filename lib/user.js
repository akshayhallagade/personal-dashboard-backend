...
function validateUserToken(token) {
  ...
  const safeparseData = validateUserTokenSchema.safeParse({ id: tokenData.id });
  if (!safeparseData) return res.json({ message: "invalid token" });
  return safeparseData.data.id;
}
